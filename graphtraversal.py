...
class Solution:
    ...
    def spath_algo(self, graph):
        ...
        max_val = 100000
        for z in unvis_nodes:
            shortest[z] = max_val
        shortest["Start"] = 0

        while unvis_nodes:
            current_min = None
            for x in unvis_nodes:
                if current_min == None:
                    current_min = x
                elif shortest[x] < shortest[current_min]:
                    current_min = x

            neighbors = []
            for y in graph[current_min]:
                neighbors.append(y)

            for z in neighbors:
                temp_val = shortest[current_min] + graph[current_min][z]
                if temp_val < shortest[z]:
                    shortest[z] = temp_val
                    prev_nodes[z] = current_min
            
            unvis_nodes.remove(current_min)
